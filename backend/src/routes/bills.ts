...
export const bills = new Hono<{ Bindings: Env }>();
...
bills.put("/:id", async (c) => {
  ...
  const existing = await c.env.DB.prepare("SELECT * FROM bills WHERE id = ?").bind(id).first<any>();
...
bills.patch("/:id/void", async (c) => {
  const id = c.req.param("id");
  await c.env.DB.prepare("UPDATE bills SET status = 'void', updated_at = datetime('now') WHERE id = ?")
    .bind(id)
    .run();
  const row = await c.env.DB.prepare("SELECT * FROM bills WHERE id = ?").bind(id).first();
  if (!row) return c.json({ error: "Bill not found" }, 404);
...
bills.delete("/:id", async (c) => {
  const id = c.req.param("id");
  const existing = await c.env.DB.prepare("SELECT id FROM bills WHERE id = ?").bind(id).first();
  if (!existing) return c.json({ error: "Bill not found" }, 404);
